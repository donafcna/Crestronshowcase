/* ===========================================================================
 * Plan3D — fond d'écran 3D de la villa (Three.js), vue d'ensemble en écorché puis zoom
 * sur la pièce affichée par le panel. Les actions de la GUI se voient dans la pièce :
 ...
 * Actif si villa_config.meta.plan3d.actif ; la disposition des pièces vient de meta.plan3d.pieces.
 * Aucune dépendance sur le reste du GUI : tout passe par CrComLib (feedbacks) et window.Plan3D.
 * =========================================================================== */
...
(function () {
    ...
    var cfg = (vc.meta && vc.meta.plan3d) || {};
    if (cfg.actif === false) return;
    var canvas = document.getElementById('plan3d-bg');
    if (!canvas) return;
    ...
    } catch (e) { console.warn('[Plan3D] WebGL indisponible', e); return; }
    ...
    function buildRoom(p) {
        ...
        switch (p.type) {
            ...
            case 'terrasse':
                ...
                tvPos = null; break;
            case 'piscine':
                box(w * 0.7, 0.6, d * 0.55, M.eau, w * 0.5, 0.05, d * 0.5, g);
    ...
    function applyLevels(R) {
        R.lamps.forEach(function (l, i) {
            var v = R.levels[i] !== undefined ? R.levels[i] : R.levels[0];
    ...
    function overviewPose() {
        ...
        var dist = (Math.max(s.x, s.z * 1.3, s.y * 1.1) * 1.12 + 4) * fitFactor();
    ...
    function loop(ts) {
        ...
        if (activeRoom) {
            activeRoom.lamps.forEach(function (l, i) { if (i < 2) { var wp = l.pos.clone().applyMatrix4(activeRoom.group.matrixWorld); roomLights[i].position.copy(wp); roomLights[i].intensity = (activeRoom.levels[i] || 0) * (i === 0 ? 55 : 30); } });
    ...
    /* ---------- Construction ---------- */
    var noms = {};
    (vc.pieces || []).forEach(function (p) { noms[p.id] = p.nom; });
    ...
    function bind() {
        if (typeof CrComLib === 'undefined' || typeof CrComLib.subscribeState !== 'function') return false;
        ...
        (function wrapBlinds(tries) {
            var f = window.animateGroupBlinds;
            if (typeof f !== 'function') { if (tries < 40) setTimeout(function () { wrapBlinds(tries + 1); }, 250); return; }
            if (f.__p3d) return;
            ...
            w.__p3d = true; window.animateGroupBlinds = w;
        })(0);
    ...
    var tries = 0; (function tryBind() { if (bind()) return; if (++tries < 40) setTimeout(tryBind, 250); })();
