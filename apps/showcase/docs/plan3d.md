...
## 17/09/2026 — tour-1 : visite résidentielle Smartphone / TSW

Le lien de secteur Residential/Résidentiel ouvre Villa Crans sur Smartphone. Le séquenceur dédié attend le GUI et la scène prêts : 3 secondes de villa assemblée, puis première pièce aléatoire à t=3 s et une autre toutes les 5 secondes (sac aléatoire, sans répétition pendant la minute). À t=60 s : TSW ; 10 secondes une fois son GUI prêt, puis Smartphone et nouvelle introduction. La dernière pièce de la minute reste donc 2 secondes (t=58 à 60). Les chargements d'iframe ne sont pas décomptés comme une présentation visible. Les adresses explicites vers un support restent respectées.

Le curseur utilise les vrais contrôles existants : liste de pièces, onglets, HVAC et volume. Il ne réécrit pas l'éclairage ou les rideaux pendant cette visite. Le visiteur conserve la priorité : appui/molette/clavier annule la visite et ses temporisations, délai de reprise 60 s sur Smartphone, 10 s ailleurs. Les autres projets gardent leur séquence générique. Aucun changement du GUI CH5 source ni de la configuration/programmes matériels.

Horloge d'environnement conservée par Showcase, y compris lorsque la 3D est démontée pour les 10 secondes TSW. Cycle 30 s jour / 10 s crépuscule / 30 s nuit / 10 s aube. Au début du crépuscule : scène CINÉMA (12/0/30/25/0 % sur les cinq premiers circuits) dans les 17 pièces, fermeture des rideaux. Au début de l'aube : OFF et ouverture des rideaux. Volets, stores et bannes ne sont pas inclus dans cette commande de rideaux. Les niveaux passent par le fondu existant de 3 secondes ; les moteurs conservent leur course animée.

`Villa.applyDemoAmbience` modifie l'état local des pièces sans les sélectionner successivement, publie les retours de la pièce courante et fournit un snapshot au plan pour les autres pièces. Ne jamais appliquer une scène en changeant successivement de pièce : cela déclencherait la caméra. Méthode protégée par `meta.mode=showcase`, sans appel Crestron matériel. Aucun média, modèle ou lumière supplémentaire.

Recette dédiée : `scripts/test-villa-tour.cjs`, entrée par le menu depuis Appartement Carouge, cycle complet en temps réel, nuit/aube/nuit, durées, changement de châssis, pointeur et reprise manuelle ; captures des trois thèmes en normal et Scène. Les bancs de rendu existants utilisent `scripts/helpers/villa-manual.cjs` pour prendre explicitement la main avant leurs assertions.
