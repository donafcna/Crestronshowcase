...
(async()=>{
 ...
 try{
  await p.goto(base+'/interfaces/residentiel/villa-gemini-frequencetv/phone');await p.waitForFunction(()=>window.__plan3d?.version==='2026-09-17-lighting-1');await p.waitForFunction(()=>window.__plan3d.navigation().phase==='room');await p.evaluate(()=>window.__plan3d.setDay(1));
