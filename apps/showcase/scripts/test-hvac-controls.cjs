...
const base=process.env.BASE_URL||'http://127.0.0.1:4201',out=path.resolve('../../Claude outputs/codex-plan3d-villa2/gui');fs.mkdirSync(out,{recursive:true});
...
async function exercise(f,device,label,theme){
 ...
 for(const group of groups)for(const j of group){await f.locator('.hvac-controls [data-join="'+j+'"]').click();await p.waitForTimeout(60);const all=await inspect(f),active=all.filter(r=>group.includes(r.join)&&r.selected);check(label+' exclusive '+j,active.length===1&&active[0].join===j);check(label+' contrast/native '+j,all.every(r=>r.selected===r.native&&contrast(r.fg,r.bg)>=4));}
 ...
 await f.locator('.hvac-controls [data-join="611"]').click();check(label+' OFF state',await f.evaluate(()=>!Villa.get('b','610')&&Villa.get('b','611')&&Villa.get('n','61')===3));await f.locator('.hvac-controls [data-join="610"]').click();
...
try{
 ...
 for(const device of ['phone','tablet','wallpanel']){
  ...
  await p.goto(base+'/interfaces/residentiel/villa-gemini-frequencetv/'+device);await p.frameLocator('iframe').locator('.hvac-controls').waitFor({state:'attached'});const f=p.frames().find(f=>f.url().includes('/showcases/'));await p.waitForTimeout(900);
 ...
 for(const file of ['iphone.html','index.html']){
  ...
  await p.goto(base+'/showcases/villa-gemini-frequencetv/'+file);await p.waitForFunction(()=>window.CrComLib&&document.querySelector('.hvac-controls .cb-btn'));
