...
- Cadrage : villa entière plein fond (derrière le châssis) ; la pièce active est cadrée dans la zone libre entre le rectangle de l'iframe et la colonne des boutons `.workspace-device-sidebar` (sinon au-dessus, sinon rien) via `api.setWindow(rect)` recalculé chaque seconde. 3D active ⇒ `.showcase-container.plan3d-on` cale le châssis au bord gauche (`left: calc(var(--chassis-w) * var(--chassis-scale) / 2 + 16px)`, `--chassis-scale` posée par `DeviceFrame`). `window.__plan3d` = API (tests Playwright : `jump()` termine les mouvements de caméra).
- Vérification : `npm run build && npx vite preview --port 4200`, page `/interfaces/residentiel/villa-gemini-frequencetv/wallpanel` : toile `.plan3d-bg-canvas` présente, aucune `.bg-video`, aucune trace de `plan3d` dans le document de l'iframe, `__plan3d.activeRoom()` suit la pièce du GUI.
