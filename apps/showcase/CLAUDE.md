...
- Cadrage : villa entière plein fond (derrière le châssis) ; la pièce active est cadrée dans la zone libre à droite du châssis (sinon au-dessus, sinon rien) via `api.setWindow(rect)` recalculé chaque seconde à partir du rectangle de l'iframe. `window.__plan3d` = API (tests Playwright : `jump()` termine les mouvements de caméra).
