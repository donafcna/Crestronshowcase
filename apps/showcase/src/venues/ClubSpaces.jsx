...
export function ClubSpaces({floor,onFloor,room,onRoom,view,onView,settings,onSettings}) {
 ...
 return <>
  ...
  <Card title={floor===0?'Réception · Grande salle':'Palier · Grande salle'}><div className="venue-choices club-room-choices">{rooms.filter(r=>r.floor===floor).map(r=><button key={r.id} aria-pressed={room===r.id} onClick={()=>onRoom(r.id)}>{r.name}</button>)}</div><p className="venue-note club-facilities">WC Hommes · WC Femmes · Escaliers</p></Card>
