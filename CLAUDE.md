...
- **Châssis (`DeviceFrame`, `scaleRules.js`, `devices.js`)** : chaque support a ses caractéristiques réelles (`model`, `diagonalInches`, `nativeW/H`, `physicalW/H` en mm). Mode normal : châssis ajusté à la page, badge « Taille réelle » seul (pas de sélecteur — l'affichage de la page est la référence validée par Donatien). Mode Scène : sélecteur *Taille réelle* (dimensions physiques calibrées par `useScreenCalibration` — diagonale + règle 100 mm dans le bandeau Dev — rognées si trop grandes, légende → F11) / *Responsive* (remplit l'espace sans plafond, légende ×k par rapport à la page).
