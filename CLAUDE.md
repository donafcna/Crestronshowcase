...
- **Vocabulaire** : « Mode Scène » = bouton d'angle du châssis (`isFullscreen` dans le code : le châssis seul dans la fenêtre du site, colonne de droite) ; « Interface seule » = bouton de la barre d'outils (GUI brute dans un nouvel onglet, dalle et tablette uniquement, pas smartphone) ; « plein écran du navigateur » = F11. Ne jamais dire « plein écran » sans préciser.
